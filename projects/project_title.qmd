...
rna_pca_prep %>%
  ...
  mutate(component = as.numeric(component)) %>%  # Convert component to numeric
  ggplot(aes(x = component, y = value)) +
  geom_col(fill = "steelblue") +
  ylab("% of total variance") +
  xlab("Principal Component (PC)") +
  scale_x_continuous(breaks = 1:6) +  # Explicit x-axis ticks: 1 to 6
  geom_text(aes(label = round(value, 1)), vjust = 1.5)

```
```{r}
ggplot(rna_pca_bake, aes(PC1, PC2, color = Group, shape = Group)) +
  geom_point(size = 4, alpha = 0.8) +
  theme_minimal() +
  labs(title = "PCA of Disomic vs Trisomic iPSCs",
       x = "PC1", y = "PC2")
